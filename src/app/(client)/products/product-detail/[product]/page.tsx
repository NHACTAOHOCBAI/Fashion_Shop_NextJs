...
export default function ProductDetail({
  ...
  const { data: reviews } = useGetReviewsByProduct(Number(product));
    ...
    toggoleFavourite(
      { productId: Number(product) },
      {
        onSuccess: () => {
          ...
          toast.success(
            `${isFavourite ? "Removed from" : "Added to"} favourite: ${
              productDetail?.name
            }`
          );
        },
    ...
    return new Set(
      filtered?.flatMap((v) =>
        v.variantAttributeValues
          .filter(
            (vav) => vav.attributeCategory.attribute.name === attributeName
          )
  ...
  return (
    <>
                  ...
                  <ToggleGroup type="single">
                    <div className="flex gap-[10px] flex-wrap">
                      {field.values.map((value) => {
                        ...
                        const isActive =
                          selectedAttributes[field.attributeName] ===
                          value.value;
                        return (
                          <ToggleGroupItem
                            key={value.id}
      ...
      <ProductReviews />
      <div>
        <h3 className="text-3xl font-bold leading-none mt-[10px]">
          RELATED PRODUCTS
        </h3>
