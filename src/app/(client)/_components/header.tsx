...
export default function Header() {
    ...
    return (
        <header className="w-full ">
            <div className="flex items-center  mx-auto w-[1200px] py-[20px]">
                <div>
                    LOGO HERE
                </div>
                            ...
                            departmentSelections?.map((department) => {
                                return (
                                    <NavigationMenuItem key={department.id} >
                                        <NavigationMenuTrigger className="bg-transparent ">{department.name}</NavigationMenuTrigger>
                                        <NavigationMenuContent>
             ...
             rounded-md overflow-hidden bg-cover bg-center bg-no-repeat"
                                                ...
                                                </li>
                                                {
                                                    department.categories.map((category) =>
                                                        <ListItem key={category.id} href={`/products/${category.slug}`} title={category.name}>
